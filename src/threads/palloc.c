...
void *palloc_get_page(enum palloc_flags flags) {
  ...
  if (page == NULL && flags == PAL_USER) {
    bool acquired = lock_try_acquire_and_block(&frame_lock);

    evict_frame();
    page = palloc_get_multiple(flags, 1);

    lock_conditional_release(&frame_lock, acquired);
  }
#endif
  return page;
}
...
void palloc_free_multiple(void *pages, size_t page_cnt) {
  ...
  if (pool == &user_pool) {
    /* If this is from the user pool, we need to check if
     * what we want to free is shared with someone else, in which
     * case we just remove ourselves from the list of owners. */
    bool acquired = lock_try_acquire_and_block(&frame_lock);

    /* Go through all the pages we want to free. */
    for (size_t i = 0; i < page_cnt; ++i) {
      struct frame_elem *f = frame_lookup((uintptr_t)vtop(i * PGSIZE + pages));

      /* If this is the last owner, then destroy.  */
      if (f == NULL || remove_from_owners(f)) {
#ifndef NDEBUG
        memset(pages, 0xcc, PGSIZE * page_cnt);
#endif
        bitmap_set_multiple(pool->used_map, page_idx, page_cnt, false);
        if (f) {
          ASSERT(hash_delete(&frame_table, &f->elem));
          free(f);
        }
      }
    }
...
/* If this is from kernel pool we free as usual. */
#ifndef NDEBUG
    memset(pages, 0xcc, PGSIZE * page_cnt);
#endif
    bitmap_set_multiple(pool->used_map, page_idx, page_cnt, false);
